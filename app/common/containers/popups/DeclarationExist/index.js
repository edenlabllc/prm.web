...
@connect(state => ({
  existingDeclaration: state.blocks.SelectedPerson.selectedPerson,
}))
@withStyles(styles)
@popup({
      ...
      >
        <div className={styles.container}>
          ...
          <div className={styles.declaration}>
            ...
            <div>
              {`Пацієнт: ${existingDeclaration.personName}`}
            </div>
            <div>
              {`з ${format(existingDeclaration.start_date)} по ${format(existingDeclaration.end_date)}`}
            </div>
            <div>
              {`Лікар: ${(existingDeclaration.doctorName)}`}
            </div>
            <div>
              {`Заклад: ${(existingDeclaration.mspName)}`}
            </div>
          </div>
        </div>
      </Popup>
